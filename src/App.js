...
function App({ signOut, user }) {
  ...
  // event detectors
  useEffect(() => {
    // on page load fetch all collections from API

    (async () => { await READ_collections(); })();
  }, []);
  ...
  return (
    <div className="App">
      <div className="Nav">
        <h3 className="Logo">Demo: AWS Amplify & GraphQL API</h3>
        <div className='Auth' onClick={ signOut }>Sign Out</div>
      </div>
            ...
            return (
              <div key={ collection.id } className="Brief">
                <div className='crud_actions'>
                  ...
                  <div className='update' onClick={ () => { PREPARE_updateCollection(collection) } }>|/|</div>
                  <div className='delete' onClick={ () => { DELETE_collection(collection.id) } }>\_/</div>
                </div>
      ...
      {/* <div className="upload_image">
        <form>
          <input type="text" name="collectionID" value={ `x` } readOnly />
          <input type="file" name="collectionImage" id="uploadImageButton" />
        </form>
      </div> */}
    </div>
  );
